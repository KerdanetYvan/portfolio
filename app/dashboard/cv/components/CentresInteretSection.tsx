...
import type { CentreInteret } from '@/db';
import { createCentreInteret, updateCentreInteret, deleteCentreInteret } from '../actions';
...
type Props = { initial: CentreInteret[] };

export default function CentresInteretSection({ initial }: Props) {
  const { showToast } = useToast();
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  const [items, setItems] = useState(initial);
  const [modal, setModal] = useState<{ mode: 'add' } | { mode: 'edit'; item: CentreInteret } | null>(null);
  const [libelle, setLibelle] = useState('');
  const [deleteId, setDeleteId] = useState<string | null>(null);

  const openAdd = () => { setLibelle(''); setModal({ mode: 'add' }); };
  const openEdit = (item: CentreInteret) => { setLibelle(item.libelle); setModal({ mode: 'edit', item }); };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (modal?.mode === 'add') {
      run(async () => { await createCentreInteret(libelle.trim()); closeModal(); }, "Centre d'intérêt ajouté");
    } else if (modal?.mode === 'edit') {
      run(async () => { await updateCentreInteret(modal.item.id, libelle.trim()); closeModal(); }, "Centre d'intérêt mis à jour");
    }
  };

  const handleDelete = () => {
    if (!deleteId) return;
    run(async () => { await deleteCentreInteret(deleteId); setDeleteId(null); setItems((prev) => prev.filter((i) => i.id !== deleteId)); }, "Centre d'intérêt supprimé");
  };
  ...
  return (
    <div className="mx-auto max-w-2xl p-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-on-surface">{"Centres d'intérêt"}</h1>
      ...
      <div className="flex flex-wrap gap-2">
        {items.length === 0 && (
          <p className="w-full py-8 text-center text-sm text-muted">{"Aucun centre d'intérêt pour le moment."}</p>
        )}
        {items.map((item) => (
          <div
            key={item.id}
          ...
          >
            ...
            <button onClick={() => openEdit(item)} className="text-muted hover:text-on-surface">
              <Pencil size={11} />
            </button>
            <button onClick={() => setDeleteId(item.id)} className="text-muted hover:text-red-400">
              <Trash2 size={11} />
            </button>
          </div>
        ))}
          ...
          >
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-on-surface">
                {modal.mode === 'add' ? "Ajouter un centre d'intérêt" : "Modifier le centre d'intérêt"}
              </h2>
      ...
      {deleteId && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="mb-2 text-sm font-semibold text-on-surface">{"Supprimer le centre d'intérêt ?"}</h2>
